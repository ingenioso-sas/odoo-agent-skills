---
name: odoo-13-controller
description: Complete reference for Odoo 13 HTTP controllers, routing, authentication types, and request/response handling.
...
# Odoo 13 Controller Guide

Complete reference for Odoo 13 HTTP controllers, routing, and request handling.
...
class DownloadController(http.Controller):
    ...
    def download_report(self, report_id):
        ...
        pdf_content, _ = report.render_qweb_pdf([report_id])

        return request.make_response(
            pdf_content,
            headers=[
                ('Content-Type', 'application/pdf'),
                ('Content-Disposition', 'attachment; filename="%s.pdf"' % report.name),
            ]
        )
...
class ApiController(http.Controller):
    ...
    def api_order_get(self, order_id):
        ...
        if not order.exists():
            # In v13 type='json' doesn't easily allow setting status code via return
            # but we can return an error in the dict
